...
public class ConversationUtils {
     ...
     * reopen. Success and fail messages can be implemented like that, as user's
     * chat options are disabled while it is in conversation.
     *
     * @param consumer       Consumer that accepts player output text.
     * @param question       Message that will be displayed in chat when player
     *                       triggers conversion.
     ...
     */
    public static void createConfirmation(Consumer<Boolean> consumer, User user, @NotNull String question,
	...
	ValidatingPrompt confirmationPrompt = new ValidatingPrompt() {
		...
		if (ArrayUtils.contains(validEntry.replaceAll("\\s", "").split(","), input.toLowerCase())) {
		    ...
		    return ConversationUtils
			    .endMessagePrompt(user.getTranslation(Constants.CONVERSATIONS + "cancelled"));
		}
	    }
	...
	new ConversationFactory(BentoBox.getInstance())
		...
		.withFirstPrompt(confirmationPrompt).withLocalEcho(false).withTimeout(90)
     ...
     *                                confirmation.
     */
    public static void createIDStringInput(Consumer<String> consumer, Function<String, Boolean> validation, User user,
     ...
     *                 conversion.
     */
    public static void createNumericInput(Consumer<Number> consumer, @NotNull User user, @NotNull String question,
     ...
     * @param user     User who is targeted with current confirmation.
     */
    public static void createStringListInput(Consumer<List<String>> consumer, User user, @NotNull String question,
	...
	new ConversationFactory(BentoBox.getInstance())
		.withPrefix(context -> user.getTranslation(Constants.CONVERSATIONS + "prefix"))
		.withFirstPrompt(stringPrompt).withModality(true).withLocalEcho(false).withTimeout(90)
		.withEscapeSequence(user.getTranslation(Constants.CONVERSATIONS + "cancel-string"))
		.addConversationAbandonedListener(ConversationUtils.getAbandonListener(consumer, user))
		.buildConversation(user.getPlayer()).begin();
    }
     ...
     * @param user     User who is targeted with current confirmation.
     */
    public static void createStringInput(Consumer<String> consumer, User user, @NotNull String question,
	    @Nullable String successMessage) {
	// Text input message.
	StringPrompt stringPrompt = new StringPrompt() {
	    @Override
